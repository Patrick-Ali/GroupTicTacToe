...
check_l_O=[] # Global varibale as part of the check function
check_l_X=[] # Global varibale as part of the check function
rol=10 # Global varibale as part of the check function

class AI():
        ###This class contains a function  which creates a random move as part of the AI's Turn###
			...
			if self.x == 0:
				for n,i in enumerate(self.x0): #Gets the index of each item in the list 
					if i==self.y: # checks to see if the index of the list is the same as the Y coordinate 
						self.x0[n]=self.Turn # Assigns the the Current turn (X or O) to the item of that index in the list
						print(self.x0,self.x1,self.x2)
						check = True # Ends the while loop once a move has been made
...
class Game():
        ###This class contains functions that check to see if the game has been won and to make a move in the 2Player and Multiplayer game modes ###
	def __init__(self,Window):
		pass

	def transform_cord(self,x):
                ###This function transforms coordinate systems and lists used in the main part of the code to how Krystian created his win function ###
	...
	def check(self):
                ###Function to check to see if somone has won ###
...
class Window(): # This class only contains functions that are directly changing or using the GUI
	...
	def __init__(self,root, port=12346): # Function defines variables to be used within the Window class 
		...
		self.colour= "black" #sets a colour of the Xs and Os on the baord
		self.x0=[0,1,2] # Board displayed as a list 
		self.x1=[0,1,2]
		self.x2=[0,1,2]
		self.ok = True #varibale that defines if it is ok to place a X or O on the board
		self.GameMode = "" # variable determining the gamemode of the game 
		self.Count = 0 #used in the Window classes functions as a temporary variable
		...
		global check_l_O, check_l_X #global lists for use in the win function
		check_l_O=[]
		check_l_X=[]
	...
	def Change_Game(self, event): # Function that changes the type of game
		self.Reset()
		self.GameMode = self.selectmode.get() #Gets the input that the user has provided via the combobox
	...
	def winnerCode(self):# this function creates a new window the display the winner
	...
	def tieCode(self):# this function creates a new window to display the result of a tie
		...
		if self.GameMode == "Multiplayer" and self.p == 2: # This is what will initaly activate player two's listening mode
			...
			if self.Turn != " ":
				...
				if self.GameMode == "2 Player":
					...
					self.ok = a
					self.Draw() 
					c = AI.Move(self) # Calls the function within the AI class to draw a move in the grid and then check if that move is already taken
					self.ok = c
					self.Draw()
					self.Count +=1
					print(self.Count)
...
if __name__ == "__main__": # Main code within the program which is not executed when the program is imported        
	root = Tk() #Variable assigned to the TK window itself so items can be created on it    
	Window= Window(root)# Calls the Window class to create the GUI
	root.mainloop()#ensures that the Tkinter event loop is being ran continuously
